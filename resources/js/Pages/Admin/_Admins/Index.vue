                ...
                <div class="-my-2 overflow-x-auto sm:-mx-6 lg:-mx-8">
                    <div class="py-2 align-middle inline-block min-w-full sm:px-6 lg:px-8">
                        <div class="my-6 flex justify-between items-center sm:px-6 lg:px-8">
                            ...
                            <button @click="showingModal = true" class="bg-blue-500 text-white py-2 px-8 shadow rounded text-sm uppercase hover:bg-blue-600">Add New</button>
                        </div>
                        <div v-if="$page.props.admins.length < 1" class="bg-orange-500 px-12 py-6 shadow flex items-center space-x-4 text-white">
                            ...
                            <p class="text-sm">There are no Admins currently in the system. Please add new using the button above</p>
                        </div>
                        <div v-else class="shadow overflow-hidden border-b border-gray-200 sm:rounded-lg">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                <tr v-for="admin in $page.props.admins" :key="admin.id" class="bg-white hover:bg-gray-50 duration-200">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <Link :href="`/admin/admins/` + admin.id">
                                            <div class="flex items-center">
                                            <div class="flex-shrink-0 h-10 w-10">
                                                <img class="h-10 w-10 rounded-full object-cover" :src="admin.profile_photo_url" alt="">
                                            </div>
                                            <div class="ml-4">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span class="px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full" :class="{'bg-red-200 text-red-900': admin.gender === 'Male', 'bg-green-200 text-green-900': admin.gender === 'Female', 'bg-yellow-200 text-yelow-900': admin.gender === 'Other'}">{{ admin.gender }}</span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">Admin</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium flex items-center justify-center">
                                        <Link :href="`/admin/admins/` + admin.id" class="text-indigo-600 hover:text-indigo-900 mr-5 flex items-center">
                                          <svg class="w-6 h-6 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>
                                          View
                                        </Link>
                                        <button @click="confirmingUserDeletion = true" class="text-red-600 hover:text-red-900 flex items-center">
                                    ...
                                    <jet-dialog-modal :show="confirmingUserDeletion">
                                      ...
                                      <template #content>
                                          Are you sure you want to delete this account? Once the account is deleted, all of its resources and data will be permanently deleted. Please confirm you would like to permanently delete this account.
                                      </template>
                                      ...
                                      <template #footer>
                                          ...
                                          <jet-danger-button class="ml-3" @click="deleteUser(admin.id)" :class="{ 'opacity-25': form.processing }" :disabled="form.processing">
                                              Delete Account
                                          </jet-danger-button>
                                      </template>
                                  </jet-dialog-modal>
            ...
            <div>
              <div class="header flex items-center justify-between bg-blue-500 px-12 py-5">
                <div class="flex items-center">
                  ...
                  <h3 class="text-white text-lg">Add Admin</h3>
                </div>
                        ...
                        <div class="px-4 py-5 bg-white sm:p-6">
                          <div class="grid grid-cols-6 gap-6">
                            ...
                            <div class="hidden col-span-6 sm:col-span-3">
                              ...
                              <input type="hidden" v-model="form.password" name="password" id="password" autocomplete="password" class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                              <jet-input-error :message="form.errors.password" class="mt-2" />
                            </div>

                            <div class="hidden col-span-6 sm:col-span-3">
                              <label for="email" class="block text-sm font-medium text-gray-700">Confirm Password</label>
                              <input type="hidden" v-model="form.password_confirmation" name="password_confirmation" id="password_confirmation" autocomplete="password_confirmation" class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
...
export default {
  ...
  methods: {
    ...
    deleteUser(id) {
      this.$inertia.delete("/admin/delete/" + id , {

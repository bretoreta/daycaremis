<template>
  <div class="flex h-screen overflow-hidden">
    ...
    <!-- Sidebar -->
    <Sidebar :role="$page.props.user.roles[0].name" :sidebarOpen="sidebarOpen" @close-sidebar="sidebarOpen = false" />
            ...
            <div class="flex flex-col">
                <div class="-my-2 overflow-x-auto sm:-mx-6 lg:-mx-8">
                    <div class="py-2 align-middle inline-block min-w-full sm:px-6 lg:px-8">
                        ...
                        <div v-if="$page.props.meals.length < 1" class="bg-orange-500 px-12 py-6 shadow flex items-center space-x-4 text-white">
                            ...
                            <p class="text-sm">There are no Meals currently in the system. Please add new using the button above</p>
                        </div>
                        <div v-else class="shadow overflow-hidden border-b border-gray-200 sm:rounded-lg">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                <tr v-for="meal in $page.props.meals" :key="meal.id" class="bg-white hover:bg-gray-50 duration-200">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <Link :href="'/' + $page.props.user.roles[0].name + `/meals/` + meal.id">
                                          <div class="text-sm font-medium text-gray-900">{{ meal.name }}</div>
                                        </Link>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="pr-3 py-1 text-sm text-gray-500 leading-5 font-semibold rounded-full" >
                                          <p><span class="font-bold rounded-full text-black bg-blue-200 text-blue-900 px-3 py-1">Breakfast</span> : {{ meal.breakfast }}</p><br>
                                          <p><span class="font-bold rounded-full text-black bg-red-200 text-red-900 px-3 py-1">Lunch</span> : {{ meal.lunch }}</p><br>
                                          <p><span class="font-bold rounded-full text-black bg-yellow-200 text-yellow-900 px-3 py-1">Supper</span> : {{ meal.supper }}</p><br>
                                          <p><span class="font-bold rounded-full text-black bg-green-200 text-green-900 px-3 py-1">Special</span> : {{ meal.special ?? 'N/A' }}</p>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <Link v-if="can.read" :href="'/' + $page.props.user.roles[0].name + `/meals/` + meal.id" class="text-indigo-600 hover:text-indigo-900 mr-5 flex items-center">
                                          <svg class="w-6 h-6 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>
                                          View
                                        </Link>
                                        <button v-if="can.delete" @click="confrimingMealDeletion = true" class="text-red-600 hover:text-red-900 flex items-center">
                                    ...
                                    <jet-dialog-modal :show="confrimingMealDeletion">
                                      <template #title>
                                          Delete Record
                                      </template>

                                      <template #content>
                                          Are you sure you want to delete this Meal? Please confirm you would like to permanently delete this record.
                                      </template>

                                      <template #footer>
                                          <jet-secondary-button @click="confrimingMealDeletion = false">
                                              Cancel
            ...
            <div>
              <div class="header flex items-center justify-between bg-blue-500 px-12 py-5">
                <div class="flex items-center">
                  ...
                  <h3 class="text-white text-lg">Add Meal</h3>
                </div>
                        ...
                        <div class="px-4 py-5 bg-white sm:p-6">
                          <div class="grid grid-cols-6 gap-6">
                            <div class="col-span-6">
                              ...
                              <input type="text" v-model="form.name" name="name" id="name" class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                            ...
                            <div class="col-span-6 sm:col-span-3">
                              ...
                              <input type="text" v-model="form.special" name="special" id="special" class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
...
export default {
  ...
  methods: {
    ...
    deleteUser(id) {
      this.$inertia.delete("/meal/delete/" + id , {
        onFinish: visit => this.confrimingMealDeletion = false
      });
      setTimeout(() => {
        this.$page.props.flash.message = null
      }, 3000);

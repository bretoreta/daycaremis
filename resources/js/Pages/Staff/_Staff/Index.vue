            ...
            <div class="flex flex-col">
                <div class="-my-2 overflow-x-auto sm:-mx-6 lg:-mx-8">
                    <div class="py-2 align-middle inline-block min-w-full sm:px-6 lg:px-8">
                        ...
                        <div v-if="$page.props.staff.length < 1" class="bg-orange-500 px-12 py-6 shadow flex items-center space-x-4 text-white">
                            ...
                            <p class="text-sm">There are no Staff currently in the system.If you're an Administrator, please add new using the button above</p>
                        </div>
                        <div v-else class="shadow overflow-hidden border-b border-gray-200 sm:rounded-lg">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                <tr v-for="staff in $page.props.staff" :key="staff.id" class="bg-white hover:bg-gray-50 duration-200">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <Link :href="'/'+ $page.props.user.roles[0].name + `/staff/` + staff.id">
                                            <div class="flex items-center">
                                            <div class="flex-shrink-0 h-10 w-10">
                                                <img class="h-10 w-10 rounded-full object-cover" :src="staff.profile_photo_url" alt="">
                                            </div>
                                            <div class="ml-4">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span class="px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full" :class="{'bg-red-200 text-red-900': staff.gender === 'Male', 'bg-green-200 text-green-900': staff.gender === 'Female', 'bg-yellow-200 text-yelow-900': staff.gender === 'Other'}">{{ staff.gender }}</span>
                                    </td>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium flex items-center justify-center">
                                        <Link v-if="can.read" :href=" '/'+ $page.props.user.roles[0].name + `/staff/` + staff.id" class="text-indigo-600 hover:text-indigo-900 mr-5 flex items-center">
                                    ...
                                    <jet-dialog-modal :show="confirmingUserDeletion">
                                      ...
                                      <template #footer>
                                          ...
                                          <jet-danger-button class="ml-3" @click="deleteUser(staff.id)" :class="{ 'opacity-25': form.processing }" :disabled="form.processing">
                                              Delete Account
                                          </jet-danger-button>
                                      </template>
                                  </jet-dialog-modal>
            ...
            <div>
              <div class="header flex items-center justify-between bg-blue-500 px-12 py-5">
                <div class="flex items-center">
                  ...
                  <h3 class="text-white text-lg">Add Staff</h3>
                </div>
...
export default {
  ...
  data() {
    return {
      ...
      url: this.$page.props.user.roles[0].name + '/staff',
  ...
  methods: {
    ...
    deleteUser(id) {
      this.$inertia.delete("/staff/delete/" + id , {

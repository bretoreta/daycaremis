                    ...
                    <div class="py-2 align-middle inline-block min-w-full sm:px-6 lg:px-8">
                        <div class="my-6 flex justify-between items-center pr-8">
                            <div class="flex items-center">
                              <Link :href="route( $page.props.user.roles[0].name + `.staff.index`)">
                              ...
                              <img :src="$page.props.staff.profile_photo_url" :alt="$page.props.staff.name" class="w-20 h-20 object-cover rounded-full mr-4">
                              <h3 class="text-3xl">{{ $page.props.staff.name }}</h3>
                              <div v-if="$page.props.user.id == $page.props.staff.id" class="font-bold ml-4 text-gray-500">(Me)</div>
                            </div>
                            <button v-if="$page.props.user.name == $page.props.staff.name" @click="showingModal = true" class="bg-blue-500 text-white py-2 px-8 shadow rounded text-sm uppercase hover:bg-blue-600">Edit</button>
                        </div>
                        ...
                        <div class="border-t border-gray-200">
                            ...
                            <div class="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                                ...
                                <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">{{ $page.props.staff.name }}</dd>
                            </div>
                            <div class="bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                                <dt class="text-sm font-medium text-gray-500">Gender</dt>
                                <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">{{ $page.props.staff.gender }}</dd>
                            </div>
                            <div class="bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                                <dt class="text-sm font-medium text-gray-500">Email address</dt>
                                <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">{{ $page.props.staff.email }}</dd>
                            </div>
                            <div class="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                                <dt class="text-sm font-medium text-gray-500">Physical Address</dt>
                                <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">{{ $page.props.staff.address }}</dd>
                            </div>
                            </dl>
                        </div>
                        </div>
            ...
            <div>
              <div class="header flex items-center justify-between bg-blue-500 px-12 py-5">
                <div class="flex items-center">
                  ...
                  <h3 class="text-white text-lg">Edit Staff</h3>
                </div>
...
<script>
  ...
  import JetInputError from '@/Jetstream/InputError.vue'
    ...
    export default {
      ...
      methods: {
        submitForm() {
          this.form.patch("/" + this.url + "/" + this.staff.id, {
